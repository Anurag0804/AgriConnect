...
// @desc    Get all order confirmations (confirmed status)
// @route   GET /api/order-confirmations
// @access  Private
router.get('/', protect, async (req, res) => {
  try {
    const orderConfirmations = await OrderConfirmation.find({})
      .populate({
        path: 'order',
    ...
    // Filter out order confirmations where the order was not 'Confirmed'
    const filteredOrderConfirmations = orderConfirmations.filter(oc => oc.order !== null);

    res.json(filteredOrderConfirmations);
  } catch (error) {
    res.status(500).json({ error: 'Server Error' });
  }
});
